...
def _forward_onnx_model(model,  # type: ModelProto
    ...
    if TEST_MODE:
        ...
        loaded_obj = np.load(current_dir_path + '/test_data/' + test_name + '/output.npy', encoding='bytes', allow_pickle=True) #type: ignore
        out = loaded_obj.item()
    else:
...
def _prepare_inputs_for_onnx(model,  # type: ModelProto
    ...
    if TEST_MODE:
        ...
        loaded_obj = np.load(dir_path + '/test_data/' + test_name + '/input.npy', encoding='bytes', allow_pickle=True) # type: ignore
